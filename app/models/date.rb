class Date
  ...
  def self.calculate(symbol)
    ...
    return @@cache[symbol] ||= Range.new(calculate_start(symbol), calculate_end(symbol))
  end
  ...
  def calculate_start(symbol)
    ...
    when :LAST_MONTH      then Date.months_ago(1).beginning_of_month
  ...
  def calculate_end(symbol)
    ...
    when :LAST_3_MONTHS   then Date.today
    when :LAST_90_DAYS    then Date.today
    when :THIS_YEAR       then Date.today
    when :LAST_YEAR       then Date.today.years_ago(1).end_of_year
    when :LAST_12_MONTHS  then Date.today
    else
      raise "Unrecognized period symbol"
    end
  end
