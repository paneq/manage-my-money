...
namespace :deploy do
  ...
  task :run_tests do
   unless fetch(:run_tests, "1") == "0"
     run "cd #{release_path} && nice -n 5 rake db:schema:load && nice -n 5 rake db:schema:load RAILS_ENV=test" #Changed
     run "cd #{release_path} && nice -n 5 rake test" #Changed by rupert so tests are not run against production database but by test database in another environment than developers have. Also priorite of task changed to 5 so it goes a bit faster than 10.
   end
  end
